...
# Agregar a cache (para que sea mas rapido, usar memoria en lugar de disco)
...
datos_entrenamiento = datos_entrenamiento.repeat().shuffle(num_ej_entrenamiento).batch(TAMANO_LOTES)
datos_pruebas = datos_pruebas.batch(TAMANO_LOTES)

import math

# Entrenar
historial = modelo.fit(datos_entrenamiento, epochs=5, steps_per_epoch=math.ceil(num_ej_entrenamiento/TAMANO_LOTES))
